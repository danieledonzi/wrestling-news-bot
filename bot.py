...
# =========================
# v80.9: inline-link embed fix + semantic career-status translation guardrail
# =========================
# Scope:
# 1) Fix Tiffany/Paige class of bugs: social/video links inside normal prose must stay inline links,
#    not become standalone oEmbed blocks. Only true standalone embed contexts become oEmbed blocks.
# 2) Add a semantic validation/repair layer for career/status wording after translation/post-edit,
#    so terms like retirement/released/cleared/status are protected as wrestling concepts instead
#    of relying on one-off string replacements.

BOT_VERSION = "v80_9_inline_embed_and_career_status_guardrail"
BOT_VERSION_FULL = f"{BOT_VERSION} ({GIT_SHA_SHORT})"


def v809_node_visible_text_without_links(node):
    try:
...
def v809_anchor_is_standalone_embed_context(node, anchor):
    """Return True only when a social/video anchor is actually being used as an embed block.

    The key rule: a link inside a normal sentence is editorial context and must remain inline.
    It becomes an embed only if the surrounding node is basically just that link or a known
    'view this post/video' wrapper.
    """
    if not node or not anchor:
        return False
    href = normalize_embed_url(anchor.get("href", ""))
    if not is_valid_embed_url(href):
        return False

    # If the node itself is a known raw/embed wrapper, accept it.
    classes = " ".join(node.get("class", []))
    if node.name in {"iframe", "amp-twitter", "amp-instagram"}:
        return True
    ...
    outside_text = v809_node_visible_text_without_links(node)
    full_low = normalize_for_check(full_text)
    link_low = normalize_for_check(link_text)
    outside_low = normalize_for_check(outside_text)

    # Pure URL paragraph, or an anchor whose visible text is just the URL.
    if V80_SOCIAL_OEMBED_RE.fullmatch(full_text or ""):
        return True
    if link_text and V80_SOCIAL_OEMBED_RE.fullmatch(link_text):
        return True

    # Typical embed wrappers generated by sites/plugins.
    embed_labels = [
        "view this post", "view this post on instagram", "guarda il post", "guarda questo post",
        "watch this video", "watch on youtube", "guarda il video", "guarda su youtube",
        "view on tiktok", "watch on tiktok", "guarda su tiktok",
        "twitter", "instagram", "tiktok", "youtube",
    ]
    if full_low and len(full_low) <= 90 and any(label in full_low for label in embed_labels):
        # Accept only when there is no meaningful prose outside the link.
        if not outside_low or len(outside_low) <= 24:
            return True

    # If the text outside the anchor contains real prose, it is an inline source/link.
    # Examples: "... al Battleground Podcast" or "... su TikTok".
    if outside_low and len(outside_low) > 24:
        return False

    # If the node is effectively just the link label, allow embed only for generic labels,
    # not named editorial sources such as "Battleground Podcast".
    generic_labels = {"instagram", "tiktok", "youtube", "x", "twitter", "video", "post", "tweet"}
    if link_low and len(full_low) <= max(len(link_low) + 12, 40):
        return any(g in link_low for g in generic_labels)

    return False


_ORIG_V809_node_social_embed_urls = _node_social_embed_urls

def _node_social_embed_urls(node):
    """v80.9: strict embed extraction.

    Keep raw embed detection, but do not promote ordinary inline social/video anchors to EMBED blocks.
    """
    if not node:
        return []
    embeds = []
    ...
    # Anchor handling is now conservative: standalone embed wrapper only.
    for a in node.find_all("a", href=True):
        href = normalize_embed_url(a.get("href", ""))
        if is_valid_embed_url(href) and v809_anchor_is_standalone_embed_context(node, a):
            embeds.append(href)

    return dedupe_preserve_order(embeds)


V809_CAREER_STATUS_SOURCE_TERMS = [
    "retirement", "retire", "retired", "semi-retired", "semi retired", "retires",
    "release", "released", "roster cuts", "future", "status", "contract", "free agent",
    "cleared", "not cleared", "injury status", "medical status", "career status",
]

V809_CAREER_STATUS_BAD_IT_TERMS = [
    "pensione", "pensionamento", "pensionata", "pensionato",
    "rilascio", "rilasciata", "rilasciato", "rilasciati", "rilasciate",
    "stato in wwe", "stato della wwe", "pulita dal punto di vista medico", "pulito dal punto di vista medico",
]


def v809_source_has_career_status_concept(source_title="", source_text=""):
    probe = normalize_for_check(f"{source_title} {(source_text or '')[:4000]}")
    return any(term in probe for term in V809_CAREER_STATUS_SOURCE_TERMS)


def v809_extract_career_status_source_excerpt(source_title="", source_text=""):
    raw = f"{source_title}\n{source_text or ''}"
    # Keep this cheap: return title plus sentences containing relevant source terms.
    pieces = []
    for sent in re.split(r"(?<=[.!?])\s+|\n+", raw):
        low = normalize_for_check(sent)
        if any(term in low for term in V809_CAREER_STATUS_SOURCE_TERMS):
...
def v809_find_bad_career_status_nodes(html="", source_title="", source_text=""):
    if not html or not v809_source_has_career_status_concept(source_title, source_text):
        return []
    bad = []
    try:
        soup = BeautifulSoup(html, "html.parser")
        for node in soup.find_all(["p", "li", "blockquote"]):
            text = sanitize_text(node.get_text(" ", strip=True))
            low = normalize_for_check(text)
            if not text:
                continue
            if any(term in low for term in V809_CAREER_STATUS_BAD_IT_TERMS):
                bad.append(text)
        # Fallback on whole text if parsing found nothing but terms exist.
        if not bad:
            full = sanitize_text(soup.get_text(" ", strip=True))
            low = normalize_for_check(full)
            if any(term in low for term in V809_CAREER_STATUS_BAD_IT_TERMS):
                bad.append(full[:500])
    except Exception:
        low = normalize_for_check(html)
        if any(term in low for term in V809_CAREER_STATUS_BAD_IT_TERMS):
            bad.append(sanitize_text(BeautifulSoup(html, "html.parser").get_text(" ", strip=True))[:500])
    return bad[:6]


def v809_deterministic_career_cleanup_sentence(sentence="", source_excerpt=""):
    """Conservative cleanup for known Italian mistranslation classes.

    This is a safety net after semantic repair, not the primary fix.
    """
    out = sentence or ""
    source_low = normalize_for_check(source_excerpt)
    if any(t in source_low for t in ["retirement", "retire", "retired", "semi retired", "semi-retired"]):
        out = re.sub(r"\bpensione\b", "ritiro", out, flags=re.I)
        out = re.sub(r"\bpensionamento\b", "ritiro", out, flags=re.I)
        out = re.sub(r"\bpensionata\b", "ritirata", out, flags=re.I)
        out = re.sub(r"\bpensionato\b", "ritirato", out, flags=re.I)
        out = re.sub(r"\bla\s+ritiro\b", "il ritiro", out, flags=re.I)
        out = re.sub(r"\bil\s+ritiro\s+non\s+e\s+(?:affatto\s+)?(?:la\s+)?situazione\b", "il ritiro non sarebbe in programma", out, flags=re.I)
        out = re.sub(r"\bil\s+ritiro\s+non\s+è\s+(?:affatto\s+)?(?:la\s+)?situazione\b", "il ritiro non sarebbe in programma", out, flags=re.I)
    if any(t in source_low for t in ["release", "released", "roster cuts"]):
        out = re.sub(r"\brilascio\b", "licenziamento", out, flags=re.I)
        out = re.sub(r"\brilasciata\b", "licenziata", out, flags=re.I)
        out = re.sub(r"\brilasciato\b", "licenziato", out, flags=re.I)
        out = re.sub(r"\brilasciati\b", "licenziati", out, flags=re.I)
        out = re.sub(r"\brilasciate\b", "licenziate", out, flags=re.I)
    return out


def v809_repair_career_status_sentence(sentence="", source_title="", source_text=""):
    source_excerpt = v809_extract_career_status_source_excerpt(source_title, source_text)
    if not sentence or not source_excerpt:
        return v809_deterministic_career_cleanup_sentence(sentence, source_excerpt)
    prompt = f"""
Sei un editor italiano di wrestling. Devi riparare UNA SOLA frase italiana che contiene probabilmente un errore su status/carriera.
Restituisci SOLO JSON valido in una riga: {{"frase":"..."}}

REGOLE:
- Mantieni lo stesso fatto dell'originale inglese.
- Nel wrestling, retirement = ritiro/ritirarsi, mai pensione/pensionamento.
- released/release = licenziato, svincolato o addio alla compagnia secondo contesto, mai rilasciato/rilascio.
- cleared/not cleared = autorizzato/non autorizzato a lottare, non "pulito".
- Non aggiungere informazioni, non cambiare nomi, eventi, date o rapporti tra soggetti.
- Riscrivi solo la frase data, in italiano naturale e giornalistico.

TITOLO ORIGINALE:
{source_title}

ESTRATTO ORIGINALE RILEVANTE:
{source_excerpt}

FRASE ITALIANA DA RIPARARE:
{sentence}
""".strip()
    try:
        data, used_model = generate_and_parse_json(prompt)
        repaired = sanitize_text(str(data.get("frase", "") or ""))
        if repaired and len(repaired) >= 12:
            repaired = v809_deterministic_career_cleanup_sentence(repaired, source_excerpt)
            print(f"[CAREER v80.9] Repair frase career/status con {used_model}")
            return repaired
    except Exception as e:
        print(f"[CAREER v80.9] Repair AI fallito, uso cleanup conservativo: {e}")
    return v809_deterministic_career_cleanup_sentence(sentence, source_excerpt)


def v809_repair_career_status_html(html="", source_title="", source_text=""):
    if not html or not v809_source_has_career_status_concept(source_title, source_text):
        return html
    bad_sentences = v809_find_bad_career_status_nodes(html, source_title, source_text)
    if not bad_sentences:
        return html
    out = html
    for bad in bad_sentences:
        repaired = v809_repair_career_status_sentence(bad, source_title, source_text)
        if repaired and repaired != bad:
            out = out.replace(bad, repaired)
    out = fix_mojibake(out)
    return out


_ORIG_V809_build_protected_facts_for_prompt = build_protected_facts_for_prompt

def build_protected_facts_for_prompt(source_title, source_text):
    facts = list(_ORIG_V809_build_protected_facts_for_prompt(source_title, source_text) or [])
    if v809_source_has_career_status_concept(source_title, source_text):
        probe = normalize_for_check(f"{source_title} {(source_text or '')[:3000]}")
        if any(t in probe for t in ["retirement", "retire", "retired", "semi retired", "semi-retired"]):
            facts.append("Career status: retirement/retired/semi-retired nel wrestling va reso come ritiro/ritirarsi/semi-ritiro, mai pensione o pensionamento.")
        if any(t in probe for t in ["release", "released", "roster cuts"]):
            facts.append("Roster status: release/released nel wrestling va reso come licenziamento, svincolo o addio alla compagnia secondo contesto, mai rilascio/rilasciato.")
        if any(t in probe for t in ["cleared", "not cleared"]):
            facts.append("Medical status: cleared/not cleared va reso come autorizzato/non autorizzato a lottare, non pulito/non pulito.")
        if any(t in probe for t in ["status", "future", "contract", "free agent"]):
            facts.append("Status/future/contract: preservare il senso editoriale sul futuro o sullo status del wrestler senza calchi letterali innaturali.")
    return dedupe_preserve_order(facts)[:18]


_ORIG_V809_v79_editorial_post_edit = v79_editorial_post_edit

def v79_editorial_post_edit(news_data, source_title="", source_text="", source_url=""):
    news_data = _ORIG_V809_v79_editorial_post_edit(news_data, source_title, source_text, source_url)
    if news_data and news_data.get("testo"):
        before = news_data.get("testo", "")
        after = v809_repair_career_status_html(before, source_title, source_text)
        if after != before:
            news_data["testo"] = after
            print("[CAREER v80.9] Guardrail career/status applicato dopo post-edit")
    return news_data


_ORIG_V809_translate_ordered_content_blocks = translate_ordered_content_blocks

def translate_ordered_content_blocks(source_title, blocks, source_url="", forced_title=None, forced_category=None, excluded_image_urls=None):
    news_data, err_type = _ORIG_V809_translate_ordered_content_blocks(
        source_title,
        blocks,
        source_url=source_url,
        forced_title=forced_title,
    ...
    if news_data and news_data.get("testo"):
        news_data["testo"] = v809_repair_career_status_html(news_data.get("testo", ""), source_title, " ".join(str(b.get("text", "")) for b in (blocks or []) if isinstance(b, dict)))
    return news_data, err_type


_ORIG_V809_translate_news = translate_news

def translate_news(source_title, text, source_url="", forced_category=None):
    news_data, err_type = _ORIG_V809_translate_news(source_title, text, source_url=source_url, forced_category=forced_category)
    if news_data and news_data.get("testo"):
        news_data["testo"] = v809_repair_career_status_html(news_data.get("testo", ""), source_title, text)
    return news_data, err_type



# =========================
# v80.10: published-only persistent HTML review archive
# =========================
# Scope:
# - Stop relying on per-run ZIP artifacts for editorial review.
# - Persist, inside the GitHub repository, only the HTML of articles actually published.
# - For each run save original source HTML and final generated HTML, plus compact metadata.
# - Keep the old all-attempted review package optional, off by default unless explicitly enabled.

BOT_VERSION = "v80_10_published_html_review_archive"
BOT_VERSION_FULL = f"{BOT_VERSION} ({GIT_SHA_SHORT})"

# Old v80.3/v80.5 all-attempted ZIP review is now optional/off by default.
# If someone explicitly sets REVIEW_PACKAGE_ENABLED=1, the old package still works.
REVIEW_PACKAGE_ENABLED = os.getenv("REVIEW_PACKAGE_ENABLED", "0").strip().lower() in {"1", "true", "yes", "on"}

PUBLISHED_HTML_REVIEW_ENABLED = os.getenv("PUBLISHED_HTML_REVIEW_ENABLED", "1").strip().lower() not in {"0", "false", "no", "off"}
PUBLISHED_HTML_REVIEW_DIR = Path(os.getenv("PUBLISHED_HTML_REVIEW_DIR", "published_html_review"))
PUBLISHED_HTML_REVIEW_MAX_CHARS = int(os.getenv("PUBLISHED_HTML_REVIEW_MAX_CHARS", "0"))  # 0 = no truncation
_PUBLISHED_HTML_REVIEW_RUN_DIR = None
_PUBLISHED_HTML_REVIEW_ITEMS = []


def _published_review_safe_slug(text, fallback="article"):
    try:
        text = normalize_for_check(str(text or ""))
    except Exception:
...
def _published_review_maybe_truncate(value):
    if value is None:
        return ""
    value = str(value)
    if PUBLISHED_HTML_REVIEW_MAX_CHARS > 0 and len(value) > PUBLISHED_HTML_REVIEW_MAX_CHARS:
        return value[:PUBLISHED_HTML_REVIEW_MAX_CHARS] + "\n\n<!-- PUBLISHED HTML REVIEW TRUNCATED -->"
    return value


def published_html_review_run_dir():
    global _PUBLISHED_HTML_REVIEW_RUN_DIR
    if not PUBLISHED_HTML_REVIEW_ENABLED:
        return None
    if _PUBLISHED_HTML_REVIEW_RUN_DIR is None:
        stamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        _PUBLISHED_HTML_REVIEW_RUN_DIR = PUBLISHED_HTML_REVIEW_DIR / f"run_{stamp}_{GIT_SHA_SHORT}"
        _PUBLISHED_HTML_REVIEW_RUN_DIR.mkdir(parents=True, exist_ok=True)
    return _PUBLISHED_HTML_REVIEW_RUN_DIR


def save_published_html_review_item(item):
    """Persist review material for published articles only.

    The folder is intentionally Git-friendly and cumulative:
      published_html_review/run_<timestamp>_<sha>/001_<slug>/
        original.html
        final.html
        metadata.json

    It does not save skipped/pending/failed candidates.
    """
    ...
    try:
        ...
        if not original_html and not final_html:
            print(f"[PUBLISHED REVIEW v80.10] Nessun HTML da salvare per: {title}")
            return
        ...
        digest_src = f"{url}|{title}|{idx}"
        digest = hashlib.sha1(digest_src.encode("utf-8", errors="ignore")).hexdigest()[:8]
        folder = base / f"{idx:03d}_{_published_review_safe_slug(title)}_{digest}"
        folder.mkdir(parents=True, exist_ok=True)

        if original_html:
            (folder / "original.html").write_text(_published_review_maybe_truncate(original_html), encoding="utf-8")
        if final_html:
            (folder / "final.html").write_text(_published_review_maybe_truncate(final_html), encoding="utf-8")
        ...
        metadata = {
            ...
            "original_html_file": "original.html" if original_html else "",
            "final_html_file": "final.html" if final_html else "",
        }
        (folder / "metadata.json").write_text(json.dumps(metadata, ensure_ascii=False, indent=2), encoding="utf-8")
        _PUBLISHED_HTML_REVIEW_ITEMS.append({"title": title, "url": url, "folder": str(folder)})
        print(f"[PUBLISHED REVIEW v80.10] Salvati HTML pubblicati: {folder}")
    except Exception as e:
        print(f"[PUBLISHED REVIEW v80.10] Errore salvataggio HTML pubblicato: {e}")
...
def finalize_published_html_review_index():
    ...
    try:
        ...
        (base / "summary.json").write_text(json.dumps(summary, ensure_ascii=False, indent=2), encoding="utf-8")
        print(f"[PUBLISHED REVIEW v80.10] Summary salvato: {base / 'summary.json'}")
    except Exception as e:
        print(f"[PUBLISHED REVIEW v80.10] Errore summary HTML pubblicati: {e}")


_ORIG_PROCESS_CANDIDATE_ITEM_V8010 = process_candidate_item

def process_candidate_item(item, history, seen_story_fingerprints, seen_news_core_keys, seen_event_keys, seen_story_signatures_v71, source_fail_counts):
    status = _ORIG_PROCESS_CANDIDATE_ITEM_V8010(
        item,
        history,
        seen_story_fingerprints,
        seen_news_core_keys,
...
V874_PUBLISHED_DIR = Path(os.getenv("PUBLISHED_DIR", "published"))
V874_MASTER_EVENTS_FILE = Path(os.getenv("MASTER_LOG_EVENTS_FILE", "logs/master_log_events.jsonl"))
V874_RUN_ARTIFACT_SUMMARY_FILE = Path(os.getenv("RUN_ARTIFACT_SUMMARY_FILE", "logs/run_artifacts_latest.json"))
V874_STRICT_REPORT_HISTORY_ENABLED = os.getenv("V874_STRICT_REPORT_HISTORY_ENABLED", "1").strip().lower() not in {"0", "false", "no", "off"}

_V874_ARTIFACT_KEYS = set()
_V874_ARTIFACT_RECORDS = []
_V874_ARTIFACT_ERRORS = []
_V874_PUBLISHED_POST_IDS = set()


def v874_safe_slug(text, fallback="article", max_len=90):
    try:
        s = sanitize_text(text or "") if "sanitize_text" in globals() else str(text or "")
    except Exception:
        s = str(text or "")
    s = s.strip().lower()
    s = re.sub(r"[\u2019'\u2018]", "", s)
    s = re.sub(r"[^a-z0-9àèéìòóùçñäöüß]+", "-", s, flags=re.I).strip("-")
    s = re.sub(r"-+", "-", s)
    return (s[:max_len].strip("-") or fallback)


def v874_atomic_write_text(path, text):
    path = Path(path)
...
def v874_atomic_write_json(path, payload):
    v874_atomic_write_text(path, json.dumps(payload, ensure_ascii=False, indent=2, default=str))


def v874_append_master_event(event):
    try:
        V874_MASTER_EVENTS_FILE.parent.mkdir(parents=True, exist_ok=True)
        event = dict(event or {})
...
def v874_artifact_key(post_id=None, source_url="", title="", sem_id=""):
    if post_id:
        return f"wp:{post_id}"
    if source_url:
        return f"url:{source_url}"
    return f"title:{title}|sem:{sem_id}"


def v874_save_published_artifacts(data, sem_id="", source_url="", result=None, event_key="", embed_urls=None, inline_images=None, featured_image_url=""):
    """Mandatory local artifact save after a successful WordPress publish.

    This restores the pre-regression behavior: every successful WP publish must
    create a local published/ artifact and a master-log event. The function is
    idempotent by wp_post_id/source_url so wrapper re-entry cannot duplicate files.
    """
    if not V874_ARTIFACT_SAVE_ENABLED:
        return True
    post_id = None
    wp_payload = None
    try:
        if isinstance(result, tuple):
            post_id = result[0]
            wp_payload = result[1] if len(result) > 1 and isinstance(result[1], dict) else None
    ...
    if isinstance(data, dict):
        ...
        html = data.get("testo") or data.get("content") or ""
        category = data.get("categoria") or data.get("category") or ""
    # Prefer tracked payload if later wrappers stored the exact final translation.
    try:
        tracked = None
        key = (str(sem_id or ""), str(source_url or ""))
        if "_V871_LAST_PUBLISH_PAYLOAD_BY_KEY" in globals():
            tracked = _V871_LAST_PUBLISH_PAYLOAD_BY_KEY.get(key)
            if not tracked:
                for (s, u), val in _V871_LAST_PUBLISH_PAYLOAD_BY_KEY.items():
                    if u and u == str(source_url or ""):
                        tracked = val
                        break
        if tracked:
            title = tracked.get("title") or title
            html = tracked.get("html") or html
            category = tracked.get("category") or category
    except Exception:
        pass
    ...
    try:
        V874_PUBLISHED_DIR.mkdir(parents=True, exist_ok=True)
        idx = len(_V874_ARTIFACT_RECORDS) + 1
        stamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        slug = v874_safe_slug(title or source_url or sem_id or f"post-{post_id}")
        base_name = f"{stamp}_{idx:03d}_{slug}_{post_id}"
        html_file = V874_PUBLISHED_DIR / f"{base_name}_final.html"
        meta_file = V874_PUBLISHED_DIR / f"{base_name}_metadata.json"
        source_file = V874_PUBLISHED_DIR / f"{base_name}_source.txt"
        ...
        meta = {
            ...
            "created_at": datetime.now().isoformat(timespec="seconds"),
            "wp_post_id": post_id,
            "wp_status": (wp_payload or {}).get("status", "publish"),
            "title": title,
            "source_url": source_url,
            "semantic_id": sem_id,
            "event_key": event_key,
        ...
        v874_atomic_write_json(meta_file, meta)
        rec = {
            "post_id": post_id,
            "title": title,
            "source_url": source_url,
...
# Outermost create_post wrapper: artifact save must happen immediately after WP success.
_ORIG_V874_create_post_without_image = create_post_without_image

def create_post_without_image(data, sem_id, url, embed_urls=None, event_key="", inline_images=None, featured_image_url=""):
    result = _ORIG_V874_create_post_without_image(
        data,
        sem_id,
        url,
        embed_urls=embed_urls,
    ...
    try:
        post_id = result[0] if isinstance(result, tuple) and result else None
        if post_id:
            ok = v874_save_published_artifacts(
                data,
                sem_id=sem_id,
                source_url=url,
                result=result,
...
# Stricten confirmed report history: only true results reports may be written.
try:
    _ORIG_V874_v872_mark_report_confirmed = v872_mark_report_confirmed
    def v872_mark_report_confirmed(report_key, source_url="", title="", wp_post_id=None, reason=""):
        if V874_STRICT_REPORT_HISTORY_ENABLED:
            probe = f"{report_key or ''} {title or ''} {source_url or ''}".lower()
            is_true = False
            try:
                is_true = bool(v865_is_true_results_report_title(title or "")) or bool(v865_is_true_results_report_event_key(report_key or ""))
            except Exception:
                is_true = False
            if not is_true:
...
def v874_finalize_run_artifacts():
    try:
        payload = {
            ...
            "git_sha": GIT_SHA,
            "created_at": datetime.now().isoformat(timespec="seconds"),
            "published_count": len(_V874_ARTIFACT_RECORDS),
            "published_post_ids": sorted(_V874_PUBLISHED_POST_IDS),
            "published_artifacts": list(_V874_ARTIFACT_RECORDS),
            "errors": list(_V874_ARTIFACT_ERRORS),
            "master_log": str(LOG_FILE),
            "master_events": str(V874_MASTER_EVENTS_FILE),
            "published_dir": str(V874_PUBLISHED_DIR),
            "published_html_review_dir": str(PUBLISHED_HTML_REVIEW_DIR) if "PUBLISHED_HTML_REVIEW_DIR" in globals() else "published_html_review",
        }
        v874_atomic_write_json(V874_RUN_ARTIFACT_SUMMARY_FILE, payload)
        v874_append_master_event({
            "type": "run_artifacts_summary",
            "published_count": len(_V874_ARTIFACT_RECORDS),
            "errors_count": len(_V874_ARTIFACT_ERRORS),
...
# Label cleanup for v87.4.
try:
    import builtins as _v874_builtins
    _ORIG_V874_PRINT = _ORIG_V873_PRINT if "_ORIG_V873_PRINT" in globals() else print
    def _v874_label_cleanup(obj):
        if not isinstance(obj, str):
            return obj
        repl = {
            ...
            "[REVIEW v87.3]": "[REVIEW v87.4]",
            "[PUBLISHED REVIEW v87.3]": "[PUBLISHED REVIEW v87.4]",
            "[BOOT v87.3]": "[BOOT v87.5]",
        }
        for old, new in repl.items():
            obj = obj.replace(old, new)
        return obj
    def _v874_print(*args, **kwargs):
        args = tuple(_v874_label_cleanup(a) for a in args)
        return _ORIG_V874_PRINT(*args, **kwargs)
    _v874_builtins.print = _v874_print
except Exception:
    pass
...
# v87.5 - report history signature crash fix
# Fixes TypeError caused by mixed legacy wrappers calling v872_mark_report_confirmed(..., url=...)
# while v87.4 exposed source_url/wp_post_id parameters. The public function is now tolerant.
try:
    _ORIG_V875_REPORT_MARKER = _ORIG_V874_v872_mark_report_confirmed if "_ORIG_V874_v872_mark_report_confirmed" in globals() else v872_mark_report_confirmed
except Exception:
    _ORIG_V875_REPORT_MARKER = None

def v875_is_strict_true_results_report(report_key="", title="", source_url=""):
    probe = f"{report_key or ''} {title or ''} {source_url or ''}".lower()
    try:
        if bool(v865_is_true_results_report_title(title or "")) or bool(v865_is_true_results_report_event_key(report_key or "")):
            return True
    except Exception:
        pass
    if not re.match(r"^report:[a-z0-9_-]+-[0-9]{4}-[0-9]{2}-[0-9]{2}$", str(report_key or "")):
        return False
    return bool(re.search(r"\b(results?|risultati|highlights?|key moments?|momenti salienti)\b", probe, re.I))

def v872_mark_report_confirmed(report_key="", title="", url="", source_url="", post_id=None, wp_post_id=None, reason="", **kwargs):
    """Backward/forward compatible confirmed-report writer.

    Accepted legacy/new names:
      - url / source_url
      - post_id / wp_post_id
    This prevents a successful WP publish from crashing during post-publish history marking.
    """
    source_url_final = source_url or url or kwargs.get("source") or kwargs.get("source_url") or ""
    post_id_final = wp_post_id if wp_post_id is not None else post_id
    if post_id_final is None:
    ...
    if _ORIG_V875_REPORT_MARKER:
        try:
            return _ORIG_V875_REPORT_MARKER(report_key, title=title, url=source_url_final, post_id=post_id_final, reason=reason)
        except TypeError:
            try:
                return _ORIG_V875_REPORT_MARKER(report_key, source_url=source_url_final, title=title, wp_post_id=post_id_final, reason=reason)
            except TypeError:
                return _ORIG_V875_REPORT_MARKER(report_key, title, source_url_final, post_id_final, reason)
    return False

# Model availability note from live ListModels. Keep routing unchanged in this hotfix,
# but expose the exact usable Gemini 3 Flash id for the next model-matrix patch.
