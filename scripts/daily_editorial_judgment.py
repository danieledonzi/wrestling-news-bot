...
def build_report(data: dict[str, Any], *, generated_at: datetime | None = None, source_artifacts_used: list[str] | None = None, missing_artifacts: list[str] | None = None) -> dict[str, Any]:
    ...
    if alfred_authoritative:
        ...
        if snap_events.get("warning_count") is not None:
    ...
    if snapshot_authoritative:
        ...
        if parsed_md.get("warnings") is not None and parsed_md.get("warnings") != warnings:
...
def render_markdown(report: dict[str, Any]) -> str:
    ...
    lines = ["# OWTV Daily Editorial Judgment 24h", "", "## Daily Editorial Judgment", "", f"- Judgment: {report['judgment']}", f"- Day type: {report['day_type']}", "", report["summary"], "", "## Daily numbers", "", f"- runs completed: {_num(report.get('runs_completed'))}", f"- news published: {_num(report.get('news_published_count'))}", f"- reports published: {_num(report.get('reports_published_count'))}", f"- article types: {dict(report['article_types']) or 'n.d.'}", f"- Menzo unique selected for downstream handoff: {_num((report.get('observability_snapshot') or {}).get('funnel', {}).get('unique', {}).get('menzo_selected_downstream'))}", f"- Diagnostic Menzo selected/pending/skipped events: {len(report['selected'])}/{len(report['pending'])}/{len(report['skipped'])}", f"- hard news count: {_num(report['hard_count'])} (stima)", f"- soft news count: {_num(report['soft_count'])} (stima)", f"- softpool used: {'yes' if report['softpool'] else 'no'}", f"- Alfred articles reviewed/articles with warnings/warning events/warning occurrences/final unique blockers: {_num(alfred_metrics.get('articles_reviewed'))}/{_num(alfred_metrics.get('articles_with_warnings'))}/{_num(alfred_metrics.get('warning_events', report['warnings']))}/{_num(alfred_metrics.get('warning_occurrences', report['warnings']))}/{_num(alfred_metrics.get('final_blockers', report['blockers']))}", f"- v95.11 duplicate arbitration: {(report.get('observability_snapshot') or {}).get('duplicate_arbitration', {})}", f"- legacy duplicate signals (diagnostic): duplicate candidates / same story clusters / same event clusters / story reviews: {_num(story['duplicate_candidates'])} / {_num(story['same_story_clusters'])} / {_num(story['same_event_clusters'])} / {_num(story['story_reviews'])}", f"- suspicious pairs above threshold: {_num(story['pairs_above_threshold'])}", f"- Menzo handoff-to-publication ratio: {ratio_label}", *gemini_lines, f"- Simone legacy errors (diagnostic; not terminal): {_num(simone_metrics.get('legacy_errors_diagnostic'))}", "", "## Hard vs soft editorial balance", "", f"Stima: {_num(report['hard_count'])} hard news e {_num(report['soft_count'])} soft news. " + ("Il ricorso al softpool sembra giustificato solo se le hard news erano limitate." if report['softpool'] else "Softpool non usato: scelta coerente se la giornata aveva sufficiente materiale hard o nessun soft recuperabile."), "", "## Top 3 discarded URLs for human control", ""]
