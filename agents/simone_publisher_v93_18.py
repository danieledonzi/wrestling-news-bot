...
VERSION = "v93_18_simone_autonomous_report_publisher"
...
def run_simone_report_publisher(simone_decision: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    print(f"[SIMONE v93.18] Avvio publisher report | ready={len(ready_reports)} wp_ok={wp_ok} dry_run={DRY_RUN}", flush=True)
    results: list[dict[str, Any]] = []
    if not wp_ok:
        results = [
            {"report_key": report_key(report), "source_url": report.get("source_url"), "title": report.get("title"), "status": "wp_not_ready", "reason": wp_status}
            for report in ready_reports if isinstance(report, dict)
        ]
    else:
    ...
    result = {
        ...
        "wp": {"ready": wp_ok, "reason": wp_status, "dry_run": DRY_RUN, "diagnostics": wp_diagnostics},
        "results": results,
        "handoff": {
        ...
        "policy": {
            "simone_is_autonomous_for_reports": True,
            "uses_manual_report_workflow_engine": "modules.report_workshop_v92.run_report_workshop",
            "checks_wp_before_scrape_or_translation": True,
            "max_reports_per_run": MAX_REPORTS_PER_RUN,
            "reports_count_as_news": False,
        },
    }
    write_json(ARTIFACT_SIMONE_PUBLISH_FILE, result)
    write_json(SIMONE_REPORT_STATUS_FILE, result)
    print(
        "[SIMONE v93.18] Report publisher completato | published={published} already={already} wp_not_ready={wp_not_ready} errors={errors}".format(
            published=result["handoff"]["published"],
            already=result["handoff"]["already_published"],
            wp_not_ready=result["handoff"]["wp_not_ready"],
            errors=result["handoff"]["errors"],
        ),
        flush=True,
    )
    return result
