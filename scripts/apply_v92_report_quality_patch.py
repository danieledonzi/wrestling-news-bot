...
if "V92_REPORT_QUALITY_PATCH = True" in text:
    print("[V92 QUALITY] patch gia applicata")
    raise SystemExit(0)

# Marker after imports/constants.
text = text.replace(
...
if old_generate not in text:
    raise SystemExit("[V92 QUALITY] generate_json block non trovato")
...
if old_prompt not in text:
    raise SystemExit("[V92 QUALITY] prompt block non trovato")
text = text.replace(old_prompt, new_prompt, 1)

# Add completion log before returning translations.
text = text.replace(
    ...
    '    if len(missing) > max(3, int(len(expected) * 0.15)):\n        raise ValueError(f"Traduzione a blocchi incompleta: mancanti={sorted(list(missing))[:20]} model={model}")\n    print(f"[TRANSLATE v92] Chain completata: report_blocks_faithful | modello={model} | blocchi_tradotti={len(translated)}/{len(expected)}", flush=True)\n    return translated\n',
    1,
)

# Insert URL normalization helper before render_blocks.
...
# Replace image rendering block with featured-image skip.
...
new_image = '''        elif btype == "image":
            ...
            if featured_image_url and normalize_media_identity(raw_src) == normalize_media_identity(featured_image_url):
                print(f"[MEDIA v92] Skip prima immagine inline gia usata come featured: {raw_src}", flush=True)
                continue
...
if old_image not in text:
    raise SystemExit("[V92 QUALITY] image render block non trovato")
text = text.replace(old_image, new_image, 1)

# Pass featured image to renderer and log scrape stats.
text = text.replace(
...
print("[V92 QUALITY] patch report quality applicata")
