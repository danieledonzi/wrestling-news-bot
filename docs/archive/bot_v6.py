...
STRONG_NAMES = [
    ...
    "kenny omega", "damian priest", "r truth", "pat mcafee",
    "kota ibushi", "joe hendry", "jacob fatu", "bayley",
...
def is_translation_coherent(source_title, generated_title):
    ...
    for src_term, gen_terms in editorial_pairs.items():
        if src_term in src_norm and any(term in gen_norm for term in gen_terms):
            translated_hits += 1

    if translated_hits >= 1:
        return True
...
'''
    ...
    try:
        ...
        report_like = any(term in source_title.lower() for term in ["report", "results", "viewership", "ratings"])
        if body_looks_suspicious(testo) and not report_like:
