...
def add_pending_article(item, reason="wp_down"):
    ...
    if score < MEDIUM_PRIORITY_SCORE:
        print(f"[PENDING] Non salvo, score basso ({score}): {item.get('title')}")
        return
...
def save_selected_candidates_to_pending(queue, reason="wp_down", limit=10):
    saved = 0
    for item in queue:
        if saved >= limit:
            break
        if int(item.get("score", 0)) >= MEDIUM_PRIORITY_SCORE:
...
def run_bot():
    ...
    if not wp_available:
        print("[BOT] WordPress offline: leggo i feed, assegno score e salvo i candidati importanti senza chiamare Gemini.")
        queue = build_candidates(history)
        save_selected_candidates_to_pending(queue, reason="wp_down_initial", limit=10)
        return
    ...
    # 1) Prima recupero pending, se presenti.
    if pending:
        print(f"[PENDING] Elementi da recuperare: {len(pending)}")
        for pitem in pending[:MAX_PENDING_TO_PROCESS_PER_RUN]:
            if time.time() - run_start > MAX_RUN_SECONDS or published_count >= MAX_POSTS_PER_RUN:
                break
            ...
            if status == "published":
                published_count += 1
            elif status == "wp_fail" or status == "wp_down":
                wp_fail_streak += 1
    ...
    # 2) Poi nuove news da feed, ordinate per score.
    queue = build_candidates(history)
    if not queue and published_count == 0:
    ...
    for idx, item in enumerate(queue):
        ...
        if validation_fail_streak >= MAX_VALIDATION_FAIL_STREAK:
            ...
            break
        if wp_fail_streak >= MAX_WP_FAIL_STREAK:
            print("[BOT] Stop anticipato: WordPress non raggiungibile, salvo candidati importanti rimanenti")
            save_selected_candidates_to_pending(queue[idx:], reason="wp_down_mid_run", limit=10)
            break
        ...
        if status == "published":
            ...
            wp_fail_streak += 1
            if wp_fail_streak >= MAX_WP_FAIL_STREAK:
                print("[BOT] WordPress non raggiungibile, salvo candidati importanti rimanenti e interrompo")
                save_selected_candidates_to_pending(queue[idx + 1:], reason="wp_down_mid_run", limit=10)
                break
        elif status == "model_fail":
            model_fail_streak += 1
        elif status == "validation_fail":
            ...
            # skip non deve interrompere la run
            pass
