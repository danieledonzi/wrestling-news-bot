...
VERSION = "v93_25_fast_wp_preflight_gate"
HEADERS = {"User-Agent": "OpenWrestlingTV-v93-WP-Preflight/1.0"}
TIMEOUT = int(os.getenv("V93_WP_PREFLIGHT_TIMEOUT", "6"))
RETRIES = int(os.getenv("V93_WP_PREFLIGHT_RETRIES", "1"))
...
def run_wp_preflight() -> dict[str, Any]:
    ...
    result: dict[str, Any] = {
        ...
        "policy": {"skip_menzo_bob_alfred_publisher_when_wp_down": True, "skip_gemini_when_wp_down": True},
    }
    ...
    for attempt in range(1, RETRIES + 1):
        for endpoint, use_auth, label in endpoints:
            ...
            if ok and label in {"rest_root", "posts_auth"}:
                ...
                write_json(WP_PREFLIGHT_FILE, result)
                write_json(ARTIFACT_WP_PREFLIGHT_FILE, result)
                print(f"[WP PREFLIGHT v93.25] ready=True reason={status}", flush=True)
                return result
        if attempt < RETRIES:
    ...
    write_json(WP_PREFLIGHT_FILE, result)
    write_json(ARTIFACT_WP_PREFLIGHT_FILE, result)
    print(f"[WP PREFLIGHT v93.25] ready=False reason={last_reason}", flush=True)
    return result
