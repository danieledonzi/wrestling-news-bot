...
def build_report(data: dict[str, Any], *, generated_at: datetime | None = None, source_artifacts_used: list[str] | None = None, missing_artifacts: list[str] | None = None) -> dict[str, Any]:
    ...
    if (section_metadata.get("andrea") or {}).get("available") is True:
        canonical_andrea = observability.get("andrea", {})
    if alfred_authoritative:
    ...
    warning_analysis_available = bool(analysis) and not (
        bool(analysis.get("errors")) and not investigations
    )
    warning_analysis = {
        "available": warning_analysis_available,
...
def render_markdown(report: dict[str, Any]) -> str:
    ...
    warning_analysis = report.get("translation_warning_analysis", {})
    warning_available = warning_analysis.get("available") is True
    warning_value = lambda key: warning_analysis.get(key, 0) if warning_available else "n.d."
    alfred_lines = [
        f"- Alfred articles reviewed/articles with warnings/final unique blockers: {_num(alfred_metrics.get('articles_reviewed'))}/{_num(alfred_metrics.get('articles_with_warnings'))}/{_num(alfred_metrics.get('final_blockers'))}",
        f"- Confirmed editorial/translation warnings: {warning_value('reproduced')}",
        f"- Warnings requiring manual verification (insufficient material): {warning_value('insufficient_material')}",
