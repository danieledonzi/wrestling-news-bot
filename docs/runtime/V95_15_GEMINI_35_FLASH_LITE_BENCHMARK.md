...
`source.json` contains common source blocks/records but excludes expected Menzo outcomes, model identity/order, prices, and tokens. Sanitization recursively removes previous published translations and WordPress metadata—including every `historical_*`, `wp_*`, and `translated_*` key—from dictionaries and lists while retaining the original source URL, English title, source HTML/text, and report blocks. Successful Bob/Menzo files expose only `{"status":"ok","output":...}`; structurally invalid results use a neutral `invalid_output` status, and failed calls use a neutral failure shape. Internal source artifacts and diagnostics—including expected-outcome checks, protected-term findings, missing IDs/indexes, and validator errors—remain exclusively in `metrics.json` and the internal report. Reviewer output therefore contains neither model identity nor automatic correctness judgments.
...
`review_template.csv` has exactly one row and one preference per comparison. It provides distinct A and B score columns. Every irrelevant score cell is prefilled with `NA`, meaning the dimension does not apply and the reviewer must not edit it; reviewers fill only blank score cells. Thus Bob and Simone rows mark all Menzo columns `NA`, Menzo rows mark all translation and severity columns `NA`, and Simone additionally marks title quality `NA` because both models receive the same deterministic title rather than generating it independently. Applicable-dimension metadata is stored only in the internal answer key. A completed preference must be exactly `A`, `B`, or `TIE`; blank, duplicated, contradictory, out-of-range, incomplete, or unmapped review data is rejected. All numeric values must be finite, so `NaN` and positive/negative infinity are invalid. When reporting, the completed CSV must cover exactly the comparison IDs in the internal answer key: missing and unexpected rows are rejected before any decision is produced. Newly generated answer keys declare `review_schema_version: 2` and use strict explicit applicability: every comparison must provide an `applicable_dimensions` list, `NA` is accepted only for dimensions absent from that list, and every listed dimension requires its normal bounded score for both outputs. The parser remains compatible with versionless and version-1 packages by validating only their historical task-specific columns and permitting unrelated columns to remain blank; historical Menzo applicability metadata continues to control `NA` within the Menzo columns without enabling schema-v2 validation.
