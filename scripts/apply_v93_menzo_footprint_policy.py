...
helper = '''def apply_source_opinion_policy(result: dict[str, Any]) -> None:
    ...
    for section in ["selected", "pending"]:
        ...
        for item in result.get(section, []) if isinstance(result.get(section), list) else []:
            ...
            if is_source_opinion(item):
                ...
                item["reason"] = "hard_skip:source_opinion_or_editorial_commentary"
    ...
    result.setdefault("postprocess", {})["source_opinion_hard_skipped"] = len(moved)


def apply_story_footprint_policy(result: dict[str, Any]) -> None:
    # Deduplicate selected + pending using durable story footprints. This runs after
    # AI normalization and after the source-opinion gate, before Bob receives URLs.
...
text = text.replace('    print(f"[MENZO v93.20] Decisione selettiva | selected={len(result.get(\'selected\', []))} pending={len(result.get(\'pending\', []))} skipped={len(result.get(\'skipped\', []))} softpool={len(load_softpool())}", flush=True)\n', '    print(f"[MENZO v93.34] Decisione selettiva | selected={len(result.get(\'selected\', []))} pending={len(result.get(\'pending\', []))} skipped={len(result.get(\'skipped\', []))} source_opinion={result.get(\'postprocess\', {}).get(\'source_opinion_hard_skipped\', 0)} footprint_dupes={result.get(\'postprocess\', {}).get(\'story_footprint_duplicates_skipped\', 0)} softpool={len(load_softpool())}", flush=True)\n')
